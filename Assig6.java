...
public class Assig6
{
	...
	public static void main(String[] args)
	{
	    ...
	    Model.CardGameFramework BUILD = new Model.CardGameFramework(
	    		numPacksPerDeck, numJokersPerPack, numUnusedCardsPerPack, 
	    		unusedCardsPerPack, numOfPlayers, cardsPerHand );
      ...
      /*
       * Build game with each component of MVC that incorporates all of the required elements
       * and is facilitated by the Control object (Controller)
       */
      Model modelBuild = new Model( BUILD, "Computer", "You" );
      
      View viewBuild = new View(); // needs to incorporate Timer object once it's created
...
class Control
{
   ...
   public Control( Model model, View view )
   {
	   ...
	   /*
	    * cards will be dealt to user on bottom pane of display as
	    * well as two stacks (cards) in the center pane
	    */
   ...
   private void playerTurn()
   {
      ...
      {
         placeNewCardsCenter();
         view.showMessage("2 Rounds of no play", 
...
{   
	...
	// panel titles
	private static String[] pnlTitles = {"BUILD", "Computer Hand", "Your Hand", "Playing Area", "Controls"};
	...
	// View constructor to set up initial view, including the card table contents and the timer
	public View()
	{
		super();
	    ...
	    pnlCntrols.setBorder( BorderFactory.createTitledBorder(pnlTitles[4]) );
	    ...
	    pnlPlayArea.setBorder( BorderFactory.createTitledBorder(pnlTitles[3]) );
	      
	    //computers hand
	    pnlCpuHand = new JPanel(new GridLayout(1, 7));
	    pnlCpuHand.setBorder( BorderFactory.createTitledBorder(pnlTitles[2]) );
	      
	    //human hand
	    pnlHumanHand = new JPanel(new GridLayout(1, 7));
	    pnlHumanHand.setBorder( BorderFactory.createTitledBorder(pnlTitles[1]) );
...
{   
	...
	public Model( CardGameFramework BUILD, String cpuName, String humanName )
	{
	    this.BUILD = BUILD;
   ...
   // Model methods for Build game data
   
   public Card playCard(int playerIndex, int cardIndex)
   {
      return BUILD.playCard(playerIndex, cardIndex);
   }
   
   public boolean takeNewCard(int playerIndex, int cardIndex)
   {
      return BUILD.takeCard(playerIndex, cardIndex);
   }
   ...
   {
      Card card = BUILD.getCardFromDeck();
      hand.takeCard(card);
   }
   ...
   {
      return BUILD.getHand(cpuPlayer);
   }
   ...
   {
      return BUILD.getHand(humanPlayer);
   }
   ...
   public int getNumCardsPerHand()
   {
      return BUILD.geNumCardsPerHand();
   }
